...
class FieldTest {
    ...
    @BeforeEach
    void setUp() {
        this.model = new ModelImpl(new FieldImpl(SIZE, this.createMines(MINES_QUANTITY)));
    }

    @Test
    void testFieldSize() {
        assertEquals(SIZE, this.model.getFieldSize());
    }

    @Test
    void testFieldMinesQuantity() {
        assertEquals(MINES_QUANTITY, this.model.getMinesQuantity());
    }
